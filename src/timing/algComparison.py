...
def plotComp(pathCsv,HEURorDPLL):
    pathCurrent = "".join([os.path.dirname(os.path.realpath(__file__)),"/../Rcode/plotComp.R"])
    subprocess.call("".join(["Rscript ",pathCurrent," ",HEURorDPLL," ",pathCsv]), shell=True)
...
##########################################
# multicore helper methods
def prepareHelper(package):

    tuple = prepare(package[0], package[1], package[2], package[3])
...
def prepare(file, path, isUDPLL, isOTHER):
    ...
    if file.endswith("cnf"):
        ...
        convertedCNF = [array.tolist() for array in cnf]
        ...
        if isUDPLL:
            ...
            return (timeCDCL, timeUDPLL, timeUDPLLple, timeOTHER)
...
def multicore_uDPLLComp(path):
    ...
    if not path.endswith("/"):
        ...
        with cf.ProcessPoolExecutor() as exec:
            list = [(file,path,True,False) for file in files]
...
def multicore_DPLLComp(path):
    ...
    if not path.endswith("/"):
        ...
        with cf.ProcessPoolExecutor() as exec:
            list = [(file,path,False,False) for file in files]
            results = exec.map(prepareHelper, list)
...
def competition(path):
    ...
    if not path.endswith("/"):
        ...
        uDPLLpleresList = np.array([])
        CDCLresList = np.array([])
        OTHERresList = np.array([])


        with cf.ProcessPoolExecutor() as exec:
            list = [(file,path,False,True) for file in files]
            results = exec.map(prepareHelper, list)

            for result in results:
                try:
                    timeCDCL, timeUDPLL, timeUDPLLple, timeOTHER = result

                    uDPLLresList = np.append(uDPLLresList,timeUDPLL)
                    uDPLLpleresList = np.append(uDPLLpleresList,timeUDPLLple)
                    CDCLresList = np.append(CDCLresList, timeCDCL)
                    OTHERresList = np.append(OTHERresList, timeOTHER)
                except:
                    print("error?")
        ...
        stats_dat.write("\n\nSummary: \n\n")
        stats_dat.write("Average time to determine satifyability in seconds: \n")
        stats_dat.write("".join(["other Team: ",str(round(np.mean(OTHERresList),decimals)), " DPLL: ",str(0),"  uDPLL: ",str(round(np.mean(uDPLLresList),decimals)),"  DPLLple: ",str(0),"  uDPLLple: ",str(round(np.mean(uDPLLpleresList),decimals)),"  CDCL: ",str(round(np.mean(CDCLresList),decimals)),"\n\n"]))
        stats_dat.write("Standard deviation of time to determine satifyability in seconds: \n")
        stats_dat.write("".join(["other Team: ",str(round(np.std(OTHERresList),decimals))," DPLL: ",str(0),"  uDPLL: ",str(round(np.std(uDPLLresList),decimals)),"  DPLLple: ",str(0),"  uDPLLple: ",str(round(np.std(uDPLLpleresList),decimals)), "  CDCL: ",str(round(np.std(CDCLresList),decimals)),"\n\n"]))
    ...
    if not path.endswith("/"):
        ...
        if len(files) >= stepCount:
            for j in range(1,stepCount):
                controlPoints = np.append(controlPoints,files[round(j*len(files)/stepCount)])
        ...
        for file in files:
            ...
            if len(files) >= stepCount:
                if file in controlPoints:
                    print("".join((str((np.where(controlPoints==file)[0][0]+1)*(100/stepCount)),"% of files processed")), end="\r")
...
def heuristicsComp(path,dpllVariant):
    ...
    # options for JWOS and JWTS are commented out since they will not be part of the final presentation
    ...
    if not path.endswith("/"):
        ...
        for file in files:
            ...
            if file.endswith("cnf"):
                ...
                if not satisfiableRAND == satisfiableMOMS == satisfiableDLCS == satisfiableDLIS:
                    pass
        ...
        stats_dat.write("".join(["RAND: ",str(round(np.mean(RANDresList),decimals)),"  MOMS: ",str(round(np.mean(MOMSresList),decimals)),"  DLCS: ",str(round(np.mean(DLCSresList),decimals)),"  DLIS: ",str(round(np.mean(DLISresList),decimals)),"\n\n"]))
        stats_dat.write("Standard deviation of time to determine satifyability in seconds: \n")
        ...
        stats_dat.write("".join(["RAND: ",str(round(np.std(RANDresList),decimals)),"  MOMS: ",str(round(np.std(MOMSresList),decimals)),"  DLCS: ",str(round(np.std(DLCSresList),decimals)),"  DLIS: ",str(round(np.std(DLISresList),decimals)),"\n\n"]))
    ...
    if len(folders)==0:
        ...
        for folder in folders:
            ...
            if os.path.isdir(folderPath):
                if len(os.listdir(folderPath))>1 and any(fname.endswith('.cnf') for fname in os.listdir(folderPath)):
                    print("".join(["processing ",folder]))
        ...
        # plot the results using an R script
        plotComp(outPath,"HEUR")
        ...
        for folder in folders:
            ...
            if os.path.isdir(folderPath):
                if len(os.listdir(folderPath))>1 and any(fname.endswith('.cnf') for fname in os.listdir(folderPath)):
                    ...
                    print("".join([folder," contains no CNFs! Continuing with next directory"]))
            else:
                print("".join(["ERROR processing ",folder,", it is not a folder! Continuing with next object"]))
        csvDat.write("")
        csvDat.close()
